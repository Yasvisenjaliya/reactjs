   ...
   <article>
     ...
     <div className=' text-white ml-[15rem] mr-[15rem] bg-slate-800 mt-4'>
      ...
      {cart?.map((item) => (
        ...
        <div key={item.id} className=' flex gap-20 border-b-2 border-green-200 '>
          <div className=' flex gap-14'>
            <img className='w-[10rem] h-24 m-2 ml-[5rem] ' src={item.img} alt={item.title} />
            <p className=' m-2'>{item.title}</p>
          </div>
          <div className=' m-2'>
            <button onClick={()=>handleChange(item , +1) } className=' border p-1 '>+</button>
            <button className='ml-2'>{item.amount} </button>
            <button onClick={()=>handleChange(item , -1) } className=' border ml-4 p-1'>-</button>
          </div>
          <div className=' m-2'>
